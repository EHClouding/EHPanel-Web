...
export function DomainsPage() {
  ...
  const [rootDomain, setRootDomain] = useState<DomainRecord | null>(null)
  const [isCreateOpen, setIsCreateOpen] = useState(false)
  const [isDirectoryOpen, setIsDirectoryOpen] = useState(false)
  const [isRootDirectoryOpen, setIsRootDirectoryOpen] = useState(false)
  ...
  const openRootModal = (item: DomainRecord) => {
    setRootDomain(item)
    setRootForm(sanitizeDocumentRoot(item.raw.document_root || "public_html"))
    setActionDomain(null)
  }

  const changeDocumentRoot = async () => {
    ...
    try {
      const documentRoot = validateDocumentRoot(rootForm || "public_html")
      const response = await hostingApi.changeDomainDocumentRoot(rootDomain.id, { document_root: documentRoot })
      setDomains((current) => current.map((domain) => (domain.id === rootDomain.id ? mapDomain(response.domain) : domain)))
      setMessage(`Raiz actualizada para ${response.domain.domain}. Reprovision enviad${response.jobs.length === 1 ? "o" : "os"} (${response.jobs.length}).`)
      setRootDomain(null)
      setRootForm("")
      await loadDomains()
    } catch (changeError) {
      setError(readMessage(changeError))
    } finally {
      setIsSaving(false)
    }
  }

  const restoreDocumentRoot = async () => {
    if (!rootDomain) return
    setError("")
    setMessage("")
    setIsSaving(true)
    ...
    try {
      const response = await hostingApi.restoreDomainDocumentRoot(rootDomain.id)
      setDomains((current) => current.map((domain) => (domain.id === rootDomain.id ? mapDomain(response.domain) : domain)))
      setMessage(`Raiz restaurada a public_html para ${response.domain.domain}.`)
      setRootDomain(null)
      setRootForm("")
      await loadDomains()
    } catch (restoreError) {
      setError(readMessage(restoreError))
    } finally {
      setIsSaving(false)
    }
  }
    ...
    <div className="space-y-4">
                ...
                <tr className="h-[52px] hover:bg-slate-50" key={item.id}>
                  ...
                  <td className="px-4 py-2">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <IconAction icon={FolderOpen} label="Cambiar raiz" onClick={() => openRootModal(item)} />
                      <button
      ...
      {actionDomain && (
        <DomainActionsModal
          ...
          onChangeRoot={() => openRootModal(actionDomain)}
        />
      )}

      {rootDomain && (
        <DocumentRootModal
          accountLabel={rootDomain.raw.account_domain || rootDomain.account}
          domain={rootDomain}
          isSaving={isSaving}
          onBrowse={() => setIsRootDirectoryOpen(true)}
          onChange={setRootForm}
          onClose={() => {
            setRootDomain(null)
            setRootForm("")
          }}
          onRestore={() => void restoreDocumentRoot()}
          onSave={() => void changeDocumentRoot()}
          value={rootForm}
        />
      )}
      ...
      {isRootDirectoryOpen && rootDomain && (
        <DirectoryPicker
          accountId={rootDomain.account}
          accountLabel={rootDomain.raw.account_domain || rootDomain.domain}
          onClose={() => setIsRootDirectoryOpen(false)}
          onSelect={(path) => {
            setRootForm(path)
            setIsRootDirectoryOpen(false)
          }}
        />
      )}
    </div>
...
function DocumentRootModal({
  ...
  const normalized = sanitizeDocumentRoot(value || "public_html")
  const current = sanitizeDocumentRoot(domain.raw.document_root || "public_html")
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 px-4">
      <div className="w-full max-w-lg rounded-lg bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
          <div>
            <div className="eh-kicker">{accountLabel}</div>
            <h3 className="mt-1 text-lg font-bold">Raiz del sitio</h3>
            <p className="mt-1 text-xs font-medium text-slate-500">{domain.domain}</p>
          </div>
          <button className="grid h-8 w-8 place-items-center rounded-md text-slate-500 hover:bg-slate-100" onClick={onClose} type="button">
            <XCircle className="h-4 w-4" />
          </button>
        ...
        <div className="space-y-4 px-5 py-4">
          <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
            <div className="text-xs font-bold uppercase text-slate-500">Ruta actual</div>
            <div className="mt-1 font-mono text-slate-900">/home/{domain.raw.account_username}/{current}</div>
          </div>

          <label className="block">
            <span className="mb-1.5 block text-xs font-bold text-slate-600">Nueva raiz</span>
            <div className="flex gap-2">
              <input
                className="h-9 min-w-0 flex-1 rounded-md border border-slate-200 px-3 font-mono text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                onChange={(event) => onChange(event.target.value)}
                placeholder="public_html"
                value={value}
              />
              <Button onClick={onBrowse} size="sm" type="button" variant="outline">
                <FolderOpen className="h-4 w-4" />
                Explorar
              </Button>
            </div>
            <span className="mt-1 block text-[11px] font-medium leading-4 text-slate-500">
              Ruta relativa dentro de la cuenta. EHPanel crea la carpeta si no existe y regenera Nginx/OpenLiteSpeed.
            </span>
          </label>

          <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
            Esto no elimina archivos ni detiene aplicaciones. Si una app Git sigue activa, desactivala desde Git/Deploy.
          </div>
        </div>

        <div className="flex flex-wrap justify-between gap-2 border-t border-slate-200 px-5 py-3">
          <Button disabled={isSaving || current === "public_html"} onClick={onRestore} size="sm" type="button" variant="outline">
            Restaurar public_html
          </Button>
          <div className="flex gap-2">
            <Button disabled={isSaving} onClick={onClose} size="sm" type="button" variant="outline">
              Cancelar
            </Button>
            <Button disabled={isSaving || !normalized || normalized === current} onClick={onSave} size="sm" type="button">
              {isSaving ? "Aplicando..." : "Guardar raiz"}
            </Button>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 px-4">
      <div className="w-full max-w-sm rounded-lg bg-white p-5 shadow-2xl">
        ...
        <div className="grid gap-2">
          ...
          <Button onClick={onChangeRoot} size="sm" variant="outline">
            <FolderOpen className="h-4 w-4" />
            Cambiar raiz
          </Button>
        </div>
