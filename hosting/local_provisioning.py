...
def helper_settings_payload():
    return {
        ...
        "postfix_tls_sni_map_file": getattr(settings, "LOCAL_POSTFIX_TLS_SNI_MAP_FILE", "/etc/postfix/ehpanel-tls-sni"),
        "postfix_tls_sni_chain_dir": getattr(settings, "LOCAL_POSTFIX_TLS_SNI_CHAIN_DIR", "/etc/postfix/ehpanel-sni-chains"),
        "dovecot_tls_sni_conf_file": getattr(settings, "LOCAL_DOVECOT_TLS_SNI_CONF_FILE", "/etc/dovecot/conf.d/99-ehpanel-sni.conf"),
...
def execute_local_job(job):
    ...
    try:
        timeout = 7200 if job.job_type == AgentJob.Type.IMPORT_MAILBOX else 1800 if job.job_type in {AgentJob.Type.DEPLOY_GIT_APP, AgentJob.Type.BACKUP_APP} else 300
